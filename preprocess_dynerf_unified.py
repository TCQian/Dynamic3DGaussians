...
class UnifiedDyNeRFPreprocessor:
    ...
    def build_camid_to_pose_index(self, folder):
        ...
        cams = [
            f for f in os.listdir(folder) if f.startswith('cam') and f.endswith('.mp4')
        ]
    ...
    def extract_all_frames(
        self, target_size=(640, 360), max_frames=150, colmap_frames_per_cam=1
    ):
        ...
        for cam_id, video_file in tqdm(video_files, desc="Extracting frames"):
            ...
            print(
                f"  Camera {cam_id:02d}: extracted {len(cam_frames)} frames ({'TRAIN' if cam_id in train_cam_ids else 'TEST'})"
            )
    ...
    def run_colmap_on_first_frame(self, target_size=(640, 360)):
        ...
        process = subprocess.Popen(
            cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=0
        )
        ...
        cmd = [
            ...
            self.images_dir,
            "--output_path",
            os.path.dirname(self.sparse_dir),
        ...
        if os.path.exists(points3d_bin):
            print(
                f"  Sparse SfM successful - found points3D.bin ({os.path.getsize(points3d_bin)/1024:.1f} KB)"
            )
        elif os.path.exists(points3d_txt):
            print(
                f"  Sparse SfM successful - found points3D.txt ({os.path.getsize(points3d_txt)/1024:.1f} KB)"
            )
        ...
        print(
            f"  Number of images: {len([f for f in os.listdir(self.images_dir) if f.endswith('.jpg')])}"
        )
        ...
        if return_code != 0:
            ...
            if os.path.exists(dense_images_dir):
                num_undistorted = len(
                    [f for f in os.listdir(dense_images_dir) if f.endswith('.jpg')]
                )
        ...
        cmd = [
            ...
            "geometric",
            "--output_path",
            os.path.join(self.dense_dir, "fused.ply"),
        ]
        print(f"  Running: {' '.join(cmd)}")
        process = subprocess.Popen(
            cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=0
        ...
        if dense_exists:
            ...
            print(
                f"  Dense reconstruction successful: {dense_ply_path} ({file_size:.1f} MB)"
            )
        else:
    ...
    def create_unified_segmentation(
        self, all_frames, train_frames, first_frame_images, cameras, poses
    ):
        """
        ...
        print(
            f"Using {len(first_frame_images)} cameras for dense point cloud generation"
        )
        ...
        if hasattr(self, 'dense_dir') and os.path.exists(self.dense_dir):
            dense_ply_path = os.path.join(self.dense_dir, "fused.ply")
            dense_ply_path = self.process_ply_file(
                dense_ply_path, dense_ply_path.replace(".ply", "_downsampled.ply")
            )
            points_colmap, colors = self.load_dense_point_cloud(dense_ply_path)
            if len(points_colmap) > 0:
                print(
                    f"  Using COLMAP dense point cloud: {len(points_colmap):,} points"
                )
        ...
        if len(points_colmap) == 0:
            ...
            if len(points_colmap) > 0:
                print(
                    f"  Using COLMAP sparse point cloud: {len(points_colmap):,} points"
                )
        ...
        print(
            f"  - Background: {np.sum(1-seg_3d):,} points ({np.mean(1-seg_3d)*100:.1f}%)"
        )
    ...
    def process_ply_file(self, input_file, output_file):
        try:
            ...
            print(
                f"  Downsampling PLY file: {input_file} containing {len(pcd.points)} points"
            )
    ...
    def load_dense_point_cloud(self, ply_path):
        ...
        try:
            ...
            for r_field, g_field, b_field in color_fields:
                if (
                    field_names
                    and r_field in field_names
                    and g_field in field_names
                    and b_field in field_names
                ):
                    ...
                    color_found = True
                    print(
                        f"    Found colors using fields: {r_field}, {g_field}, {b_field}"
                    )
                    break
            ...
            print(
                f"  Successfully loaded {len(points):,} dense points from PLY using PlyData"
            )
            print(
                f"  Dense point cloud bounds: X=[{points[:,0].min():.2f}, {points[:,0].max():.2f}]"
            )
            print(
                f"                           Y=[{points[:,1].min():.2f}, {points[:,1].max():.2f}]"
            )
            ...
            print(
                f"  Color range: R=[{colors[:,0].min():.3f}, {colors[:,0].max():.3f}]"
            )
            print(
            ...
            print(
                f"  Error: PlyData not available. Please install with: pip install plyfile"
            )
            return np.array([]), np.array([])
            ...
            with open(images_path, 'rb') as f:
                ...
                for _ in range(num_images):
                    ...
                    if camera_id in cameras:
                        cameras[camera_id]['images'] = cameras[camera_id].get(
                            'images', {}
                        )
        ...
        for cam_id in sorted(train_cam_ids):
            if cam_id in self.camid_to_pose_index:
                ...
                if pose_idx < len(poses):
                    ...
                    camera_center = c2w[
                        :3, 3
                    ]  # Translation is camera center in world coords
                    camera_centers_llff.append(camera_center)
                    cam_id_mapping.append(cam_id)
                    print(
                        f"    LLFF cam {cam_id} (pose idx {pose_idx}): center = [{camera_center[0]:.3f}, {camera_center[1]:.3f}, {camera_center[2]:.3f}]"
                    )
                else:
                    print(
                        f"    Warning: pose index {pose_idx} for cam {cam_id} >= len(poses) {len(poses)}"
                    )
            else:
        ...
        for cam_id in sorted(train_cam_ids):
            ...
            for colmap_cam_id, cam_info in cameras.items():
                if 'images' in cam_info and colmap_image_name in cam_info['images']:
                    ...
                    cam_id_mapping.append(cam_id)
                    print(
                        f"    COLMAP cam {cam_id}: center = [{camera_center[0]:.3f}, {camera_center[1]:.3f}, {camera_center[2]:.3f}]"
                    )
                    found = True
                    break
    ...
    def compute_procrustes_alignment(self, points_src, points_dst):
        ...
        if len(points_src) != len(points_dst) or len(points_src) < 3:
            print(
                f"    Error: Need at least 3 corresponding points, got {len(points_src)}"
            )
    ...
    def align_colmap_to_llff(self, poses, cameras, train_cam_ids):
        ...
        centers_llff, llff_cam_ids = self.extract_camera_centers_from_llff(
            poses, train_cam_ids
        )
        centers_colmap, colmap_cam_ids = self.extract_camera_centers_from_colmap(
            cameras, train_cam_ids
        )
        ...
        if len(common_cam_ids) < 3:
            print(
                f"  Warning: Only {len(common_cam_ids)} common cameras found, need at least 3 for robust alignment"
            )
            return 1.0, np.eye(3), np.zeros(3)
        ...
        print(
            f"  Using {len(common_cam_ids)} common cameras for alignment: {common_cam_ids}"
        )

        # Compute alignment
        scale, R, t = self.compute_procrustes_alignment(
            centers_colmap_matched, centers_llff_matched
        )
    ...
    def transform_dense_cloud_to_llff(self, points, colors, scale, R, t):
        ...
        print(
            f"    Original bounds: X=[{points[:,0].min():.2f}, {points[:,0].max():.2f}]"
        )
        print(
            f"                    Y=[{points[:,1].min():.2f}, {points[:,1].max():.2f}]"
        )
        ...
        print(
            f"    LLFF bounds:    X=[{points_llff[:,0].min():.2f}, {points_llff[:,0].max():.2f}]"
        )
        print(
            f"                    Y=[{points_llff[:,1].min():.2f}, {points_llff[:,1].max():.2f}]"
        )
        print(
            f"                    Z=[{points_llff[:,2].min():.2f}, {points_llff[:,2].max():.2f}]"
        )
    ...
    def get_llff_w2c_matrix(self, cam_id, poses):
        ...
        if cam_id in self.camid_to_pose_index:
            ...
            if pose_idx < len(poses):
                ...
                print(
                    f"    Warning: pose index {pose_idx} for cam {cam_id} >= len(poses) {len(poses)}, using identity"
                )
                return np.eye(4)
        else:
            print(
                f"    Warning: cam_id {cam_id} not found in camera mapping, using identity"
            )
            return np.eye(4)

    def create_metadata_colmap_centric(
        self, cameras, train_frames, test_frames, target_size
    ):
        ...
        def build_metadata(cam_ids, frames_dict, metadata_type):
            ...
            for t in range(T):
                ...
                for cam_id in sorted(cam_ids):
                    ...
                    if K is None or R is None or t_vec is None:
                        ...
                        K = np.array(
                            [
                                [w_target, 0, w_target / 2],
                                [0, h_target, h_target / 2],
                                [0, 0, 1],
                            ]
                        )
                        R = np.eye(3)
            ...
            print(
                f"Created {metadata_type} metadata: {len(meta['fn'])} timesteps, {len(cam_ids)} cameras (COLMAP)"
            )
            return meta
    ...
    def run_unified_preprocessing(self, target_size=(640, 360), max_frames=300):
        ...
        colmap_frames_per_cam = (
            3  # Use 3 frames per camera for better dense reconstruction
        )
        all_frames, train_frames, test_frames, first_frame_images = (
            self.extract_all_frames(target_size, max_frames, colmap_frames_per_cam)
        )
        ...
        self.create_unified_segmentation(
            all_frames, train_frames, first_frame_images, cameras, None
        )

        # Create metadata (separate train and test) with TARGET dimensions - COLMAP-centric
        if cameras is None:
            print(
                "Warning: COLMAP cameras not found; metadata will use fallback intrinsics/extrinsics"
            )
        self.create_metadata_colmap_centric(
            cameras if cameras is not None else {},
            train_frames,
            test_frames,
            target_size,
        )
        ...
        print(
            " Separate train_meta.json and test_meta.json created (fixed cameras per timestep)"
        )
...
def main():
    ...
    success = preprocessor.run_unified_preprocessing(
        target_size=(args.width, args.height), max_frames=args.max_frames
    )

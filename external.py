...
def densify_light(params, variables, optimizer, i):
    """Lighter densification for subsequent timesteps in dynamic scenes"""
    if i <= 2000:  # Shorter densification period
        variables = accumulate_mean2d_gradient(variables)
        grad_thresh = 0.0004  # Higher threshold (less aggressive)
        if (i >= 200) and (i % 200 == 0):  # Less frequent
            grads = variables['means2D_gradient_accum'] / variables['denom']
            grads[grads.isnan()] = 0.0

            # Only clone small gaussians with high gradients (no splitting)
            to_clone = torch.logical_and(
                grads >= grad_thresh,
                (
                    torch.max(torch.exp(params['log_scales']), dim=1).values
                    <= 0.015 * variables['scene_radius']
                ),
            )

            if to_clone.sum() > 0:  # Only if there are points to clone
                new_params = {
                    k: v[to_clone]
                    for k, v in params.items()
                    if k not in ['cam_m', 'cam_c']
            ...
            # Remove low opacity points more aggressively
            remove_threshold = 0.1  # Higher threshold than original (0.005)
            to_remove = (
                torch.sigmoid(params['logit_opacities']) < remove_threshold
            ).squeeze()

            # Also remove points that are too big
            if i >= 1000:
                big_points_ws = (
                    torch.exp(params['log_scales']).max(dim=1).values
                    > 0.08 * variables['scene_radius']
                )
                to_remove = torch.logical_or(to_remove, big_points_ws)

            if to_remove.sum() > 0:  # Only if there are points to remove
